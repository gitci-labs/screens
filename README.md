...
.build/debug/gitci-screens validate examples/minimal --strict
.build/debug/gitci-screens validate examples/minimal --pseudo-locale
.build/debug/gitci-screens validate examples/minimal --overflow-locale
...
.build/debug/gitci-screens build examples/minimal --scene-set launch --variant-group baseline
.build/debug/gitci-screens export examples/minimal --scene-set launch --strict --fastlane-out fastlane/screenshots
.build/debug/gitci-screens export examples/minimal --scene-set launch --all-variant-groups --fastlane-out fastlane/screenshots
.build/debug/gitci-screens archive examples/minimal --scene-set launch
.build/debug/gitci-screens fastlane examples/minimal --scene-set launch --out fastlane/screenshots
.build/debug/gitci-screens gallery examples/minimal --scene-set launch
.build/debug/gitci-screens init path/to/my-app --name "My App" --github-workflow
.build/debug/gitci-screens scene-sets create path/to/my-app --template gitci.core.basic-launch --id launch --variant hero=device --asset hero=path/to/hero.png
.build/debug/gitci-screens scene-sets fill-assets path/to/my-app --scene-set launch --asset detail=path/to/detail.png
...
```sh
docker run --rm -v "$PWD":/workspace ghcr.io/gitci-labs/screens:main export /workspace/examples/minimal --strict
```
...
`build` writes `manifest.gitci-output.json` next to the render plan. The output manifest includes each screenshot's target dimensions plus locale metadata, span, span index, composite canvas, display gap, and clip rectangle metadata so galleries and future editors can reconstruct localized and wide scenes precisely. `gallery` writes a static HTML index under the selected scene set build directory with filters for target, locale, and variant group, and `archive` zips an existing scene-set build directory for CI artifacts or sharing. `fastlane` copies an existing build into a `fastlane/screenshots/<locale>/...` layout for `fastlane deliver`; `export --fastlane-out fastlane/screenshots` does the same after rendering. `export` runs build, gallery, and archive in one pass for local dogfooding and CI. Pass `--strict` to `validate`, `build`, or `export` when warnings such as first-three screenshots missing headlines should fail the run. Pass `--pseudo-locale` to `validate`, `plan`, `build`, or `export` to add a synthetic `qps-ploc` locale with expanded pseudo-localized strings for layout stress testing. Pass `--overflow-locale` to add a synthetic `qps-overflow` locale with deliberately long localized strings for stress renders, and combine it with `--fail-on-overflow` on `build` or `export` to fail when the renderer detects clipped DOM overflow. Pass `--variant-group <id>` to build named slot selections for PPO/custom-product-page experiments under `build/<scene-set>/<variant-group>/`; `validate --all-variant-groups` checks every declared group, and `export --all-variant-groups` renders every group with default per-group output paths plus a `variant-groups.gitci-export.json` index. When `--all-variant-groups` is combined with `--fastlane-out fastlane/screenshots`, each group gets a separate Fastlane root such as `fastlane/screenshots/baseline/<locale>/...`. Variant group selections can use `slot-id@target-pattern` keys when the iPhone, iPad, and Mac outputs need different variants in the same experiment.
...
- pseudo-locale output for localization layout stress testing
- overflow-locale output for long-string stress renders
- named variant groups for product-page experiments
- Fastlane screenshots folder export for App Store Connect upload pipelines
