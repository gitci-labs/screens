...
- 2026 comparison pages are now repeatedly positioning AppScreens, AppScreenshotStudio, AppLaunchpad, Screenshots Pro, StoreShots/Storeshots, LaunchShot, and Figma/Canva workflows along an effort-to-output spectrum. The recurring product promises are exact store sizes, large template libraries, AI-assisted copy/layout, localization at scale, and one-click/bulk export: https://appscreens.com/blog/best-app-store-screenshot-tools-2026, https://appscreenshotstudio.com/blog/proven-2026-guide-top-7-app-store-screenshot-generators, https://storeshots.org/, and https://launchshot.app/
...
- Apple's App Store product page guidance says up to 10 screenshots can be featured and the first one to three can appear in search results when there is no app preview. Its Product Page Optimization docs support testing up to three alternate versions against the original. That keeps the need for data-driven target profiles, first-three screenshot ordering, and screenshot/app-preview experiments current: https://developer.apple.com/app-store/product-page-updates/, https://developer.apple.com/help/app-store-connect/reference/screenshot-specifications, and https://developer.apple.com/app-store/product-page-optimization/
...
1. Keep improving visual overflow detection on top of the current `qps-overflow` long-string stress locale; `--fail-on-overflow` now catches clipped DOM overflow during Playwright rendering, but canvas/WebGL-aware checks still need coverage.
2. Improve variant groups into a reviewable experiment surface: all-groups validation, grouped Fastlane roots, first-three gallery badges, per-group archive names, and an all-groups export index are in place; next add custom product page packaging conventions.
...
5. Continue hardening the GUI import/build loop: persisted local directory and git clone import, scene-set build actions, gallery preview, reveal/share output, then template source bootstrap/update.
